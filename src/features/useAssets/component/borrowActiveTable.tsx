...
export default function BorrowActiveTable({
  ...
  if (!data.length) {
    return <div className="text-sm text-gray-600">Tidak ada peminjaman yang sedang berlangsung.</div>;
  }
        ...
        <tbody className="text-gray-700">
          {pageData.map((r) => (
            <tr key={r.id_asset_borrowed} className="border-t">
              ...
              <td className="px-4 py-2">{r.status === "DIPAKAI" ? "Kantor" : (r.user?.name ?? r.id_user ?? "-")}</td>
