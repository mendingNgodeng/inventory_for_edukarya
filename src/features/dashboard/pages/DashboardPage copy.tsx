...
import { Package, MapPin, Tag, TrendingUp, Clock, AlertCircle, RefreshCw } from 'lucide-react';
import DashboardLayout from '../../../layouts/Dashboardlayout';
import { useDashboard } from '../../../api/statistic/hooks';

const DashboardPage: React.FC = () => {
  const { summary, categoryRanking, loading ,error,fetchDashboard} = useDashboard();
...
const maxStock =
  categoryRanking.length > 0
    ? Math.max(...categoryRanking.map(c => c.total_stock))
    : 0;

const assetCategories = categoryRanking.map((cat) => ({
  category: cat.name,
  count: cat.total_stock,
  percentage: maxStock
