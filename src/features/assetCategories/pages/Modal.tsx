...
import type { CtgFormData, CtgModalProps } from './Types';

const LocationModal: React.FC<CtgModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
  editingData,
...
const {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={editingData ? 'Edit Kategori' : 'Tambah Kategori'}
      size="md"
      footer={
  ...
  try {
    ...
    if (error.response?.status === 400) {
      ...
      Object.keys(backendErrors).forEach((field) => {
        setError(field as keyof CtgFormData, {
          type: "server",
